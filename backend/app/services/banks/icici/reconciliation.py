"""Phase 3: icici reconciliation wraps shared pipeline.reconciliation."""
...
class ICICIReconciliationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedReconciliationResult) -> "ICICIReconciliationResult":
        return cls(
...
class ICICIReconciliation:
    ...
    def reconcile(
        ...
        if not transactions:
            raise ICICIReconciliationError(
                "No transactions to reconcile", error_code="NO_TRANSACTIONS"
            )
        self.logger.info("Reconciling %d ICICI Bank transactions", len(transactions))
        ...
        result = ICICIReconciliationResult.from_shared(shared)
        self.logger.info(
