"""Phase 4: idfc aggregation wraps shared pipeline.aggregation."""
...
class IDFCAggregationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedAggregationResult) -> "IDFCAggregationResult":
        return cls(
...
class IDFCAggregationEngine:
    ...
    ) -> IDFCAggregationResult:
        shared = self._engine.aggregate(
            transactions,
            opening_balance=opening_balance,
            closing_balance=closing_balance,
        )
        return IDFCAggregationResult.from_shared(shared)
