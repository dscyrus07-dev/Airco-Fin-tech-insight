"""Phase 3: idfc reconciliation wraps shared pipeline.reconciliation."""
...
class IDFCReconciliationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedReconciliationResult) -> "IDFCReconciliationResult":
        return cls(
...
class IDFCReconciliation:
    ...
    def reconcile(
    ...
    ) -> IDFCReconciliationResult:
        if not transactions:
            raise IDFCReconciliationError(
                "No transactions to reconcile", error_code="NO_TRANSACTIONS"
            )
        self.logger.info("Reconciling %d IDFC Bank transactions", len(transactions))
        ...
        result = IDFCReconciliationResult.from_shared(shared)
        self.logger.info(
