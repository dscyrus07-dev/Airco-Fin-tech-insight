"""Phase 4: karnataka aggregation wraps shared pipeline.aggregation."""
...
@dataclass
class KarnatakaAggregationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedAggregationResult) -> "KarnatakaAggregationResult":
        return cls(
...
class KarnatakaAggregationEngine:
    ...
    ) -> KarnatakaAggregationResult:
        shared = self._engine.aggregate(
            transactions,
            opening_balance=opening_balance,
            closing_balance=closing_balance,
        )
        return KarnatakaAggregationResult.from_shared(shared)
