"""Phase 3: kotak reconciliation wraps shared pipeline.reconciliation."""
...
class KotakReconciliationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedReconciliationResult) -> "KotakReconciliationResult":
        return cls(
...
class KotakReconciliation:
    ...
    def reconcile(
        ...
        if not transactions:
            raise KotakReconciliationError(
                "No transactions to reconcile", error_code="NO_TRANSACTIONS"
            )
        self.logger.info("Reconciling %d Kotak Bank transactions", len(transactions))
        ...
        result = KotakReconciliationResult.from_shared(shared)
        self.logger.info(
