"""Phase 3: paytm reconciliation wraps shared pipeline.reconciliation."""
...
class PaytmReconciliationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedReconciliationResult) -> "PaytmReconciliationResult":
        return cls(
...
class PaytmReconciliation:
    ...
    def reconcile(
    ...
    ) -> PaytmReconciliationResult:
        if not transactions:
            raise PaytmReconciliationError(
                "No transactions to reconcile", error_code="NO_TRANSACTIONS"
            )
        self.logger.info("Reconciling %d Paytm Bank transactions", len(transactions))
        ...
        result = PaytmReconciliationResult.from_shared(shared)
        self.logger.info(
