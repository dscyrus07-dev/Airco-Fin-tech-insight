"""Phase 4: sbi aggregation wraps shared pipeline.aggregation."""
...
class SBIAggregationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedAggregationResult) -> "SBIAggregationResult":
        return cls(
...
class SBIAggregationEngine:
    ...
    ) -> SBIAggregationResult:
        shared = self._engine.aggregate(
            transactions,
            opening_balance=opening_balance,
            closing_balance=closing_balance,
        )
        return SBIAggregationResult.from_shared(shared)
