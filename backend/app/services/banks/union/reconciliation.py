"""Phase 3: union reconciliation wraps shared pipeline.reconciliation."""
...
class UnionReconciliationResult:
    ...
    @classmethod
    def from_shared(cls, shared: SharedReconciliationResult) -> "UnionReconciliationResult":
        return cls(
...
class UnionReconciliation:
    ...
    def reconcile(
    ...
    ) -> UnionReconciliationResult:
        if not transactions:
            raise UnionReconciliationError(
                "No transactions to reconcile", error_code="NO_TRANSACTIONS"
            )
        self.logger.info("Reconciling %d Union Bank transactions", len(transactions))
        ...
        result = UnionReconciliationResult.from_shared(shared)
        self.logger.info(
